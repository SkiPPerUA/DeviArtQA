...
@Test
public class ProcessingCall_reminder extends BaseTest {
    ...
    public void approve_test() throws SQLException {
        processingPage.setCallResult()
                .setCallResult(ModalWindow.CallResult.approve)
                .setComment("comment")
                .setOperatorStatus(ModalWindow.OperatorStatus.stopProcessing)
                .save(true);
        ...
        res.next();
        Assert.assertEquals(res.getInt("call_sequence_type"),2);
    ...
    void auth() throws SQLException, InterruptedException {
        for (int i = 0; i<10; i++) {
            ...
            Thread.sleep(1000);
            getDB().update("update terraleads_shipping.`call` set call_sequence_type = 2, order_id = " +
                    "(SELECT id FROM terraleads_shipping.`order` order by id desc limit 1) where lead_id = "+lead);
            getDB().update("update terraleads_shipping.call_sequence set type = 2 where lead_id = "+lead);
        }
        new WelcomePage().open().header.changeUser(25623);
    }
