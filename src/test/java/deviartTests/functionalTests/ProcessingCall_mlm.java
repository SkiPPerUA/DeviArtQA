...
@Test
public class ProcessingCall_mlm extends BaseTest {
    ...
    void auth() throws SQLException, InterruptedException {
        for (int i = 0; i<10; i++) {
            ...
            Thread.sleep(1000);
            getDB().update("update terraleads_shipping.`call` set call_sequence_type = 9 where lead_id = "+lead);
            getDB().update("update terraleads_shipping.call_sequence set type = 9 where lead_id = "+lead);
        }
        getDB().update("update terraleads_shipping.call_queue_ref_operator set operator_id = 25555 where operator_id = 25562");
        getDB().update("update terraleads_shipping.call_queue_ref_operator set operator_id = 25562 where call_queue_id = 27");
