...
public class StatisticsTest extends BaseTest {
    ...
    public void checkLinks() throws InterruptedException {
        statisticsPage.open("web")
                .readyPage()
                .setDate_from("2025-09-23") //2024-07-10
                .setDate_to("2025-09-25")
                .clickShowButton();
        Thread.sleep(3000);
        Widget widget = new Widget(Session.getPage().locator("//table[@role='grid']//a"));
        for (int i = 0; i < 100; i++){
            if (i < widget.element.count()) {
                statisticsPage.open("web")
                        .readyPage()
                        .setDate_from("2025-09-23")
                        .setDate_to("2025-09-25")
                        .clickShowButton();
                checkLink(widget,i);
            }
    ...
    public void checkResult() throws InterruptedException {
        ...
        String checkLoc = "//button[@data-id='group_mode']";
        statisticsPage.open(type).readyPage();
...
//        String group = "Offer";
//
//        statisticsPage.open(type)
//                .readyPage()
//                .setDate_from("2025-09-05")
//                .setDate_to("2025-09-05")
//                .setGroupBy(group)
//                //.setTimezone("+03")
//                .clickShowButton();
//        Thread.sleep(3000);
//        List<String> old_data = getData(false);
...
//                .clickShowButton();
//        Thread.sleep(3000);
//        List<String> new_data = getData(true);
//        try {
...
//        }
    ...
    public void findErrorFilter(){
        String pageType = "web";
        List<String> fill = List.of("dest_type","offer_type","group_mode","price_type","group_mode_day");//"timezone_offset"
        //List<String> fill = List.of("offer_id","country_group_id","manager_id","product_base_category_id","user_group_id","country_id");
    ...
    private void ress(String type, String group) throws InterruptedException {
        //String group = "Offer";
    ...
    private List<String> getData(boolean newTable){
        List<String> data = new ArrayList<>();
        Widget widget;
        if (newTable){
            widget = new Widget(Session.getPage().locator("//div[@id='statistic-table']//td"));
        }else {
            widget = new Widget(Session.getPage().locator("//table[@id='statistic-table-data']//td"));
        }
