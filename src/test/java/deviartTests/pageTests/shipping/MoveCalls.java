...
public class MoveCalls extends BaseTest {
    ...
    public void positive_move() {
        ...
        call_sequence_types.forEach(x->{
            getDB().update("UPDATE terraleads_shipping.`call` SET call_sequence_type = "+(call_sequence_types.indexOf(x)+1)+", operator_id = "+operator_from+" WHERE id = "+call_id);
        ...
        //revert
        call_sequence_types.forEach(x->{
            getDB().update("UPDATE terraleads_shipping.`call` SET call_sequence_type = "+(call_sequence_types.indexOf(x)+1)+", operator_id = "+operator_to+" WHERE id = "+call_id);
            callTransferPage.open().readyPage()
